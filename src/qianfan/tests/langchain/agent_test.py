...
from middlelayer.agents import QianfanSingleActionAgent, QianfanMultiActionAgent
...
def paper_search(query: str, **kwargs) -> str:
    ...
    if not query == "physics":
        raise ValueError(f"value retrieved from mock server isn't physics, rather: {query}")
...
def test_sync_qianfan_single_agent():
    tools = [paper_search]
    agent_qianfan = QianfanSingleActionAgent.from_system_prompt(tools, QianfanChatEndpoint(model="ERNIE-Bot-4"))
...
async def test_async_qianfan_single_agent():
    ...
    agent_qianfan = QianfanSingleActionAgent.from_system_prompt(tools, QianfanChatEndpoint(model="ERNIE-Bot-4"))
    agent_executor_qianfan = AgentExecutor(agent=agent_qianfan, tools=tools)
    assert (await agent_executor_qianfan.arun("帮我搜索一篇 physics 领域的论文")) == "测试成功"


def test_sync_qianfan_multi_agent():
    tools = [paper_search]
    agent_qianfan = QianfanMultiActionAgent.from_system_prompt(tools, QianfanChatEndpoint(model="ERNIE-Bot-4"))
    agent_executor_qianfan = AgentExecutor(agent=agent_qianfan, tools=tools)
    assert agent_executor_qianfan.run("帮我搜索一篇 physics 领域的论文") == "测试成功"
