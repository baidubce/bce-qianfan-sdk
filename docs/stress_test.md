...
- **first_latency_threshold (float)**：指定首句时延的阈值，单位为秒；
- **round_latency_threshold (float)**：指定全长时延的阈值，单位为秒；
- **success_rate_threshold (float)**：指定请求成功率的阈值，单位为百分比；
