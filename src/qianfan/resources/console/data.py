...
from qianfan.resources.console.user_constant import DataProjectType, DataTemplateType, DataSetType, DataStorageType, \
    DataSourceType, DataZipInnerContentFormatType, DataExportScene
...
class Data:
    ...
    def create_bare_dataset(
        ...
        if storage_type == DataStorageType.PrivateBos:
            if not storage_id:
                raise ValueError("storage id is empty while create dataset in private bos")
            if not storage_path:
                raise ValueError("storage path is empty while create dataset in private bos")
    ...
    @classmethod
    @console_api_request
    def release_dataset(cls, dataset_id: int, **kwargs) -> QfRequest:
    ...
    @classmethod
    @console_api_request
    def get_dataset_info(cls, dataset_id: int, **kwargs) -> QfRequest:
    ...
    @classmethod
    @console_api_request
    def get_dataset_status_in_batch(cls, dataset_id_list: List[int], **kwargs) -> QfRequest:
        dataset_id_list: List[int] = list(map(lambda id: str(id), dataset_id_list)) # noqa
        ids = ",".join(dataset_id_list) # noqa
    ...
    @classmethod
    @console_api_request
    def delete_dataset(cls, dataset_id: int, **kwargs) -> QfRequest:
    ...
    @classmethod
    @console_api_request
    def get_dataset_export_records(cls, dataset_id: int, **kwargs) -> QfRequest:
    ...
    @classmethod
    @console_api_request
    def get_dataset_error_detail(cls, dataset_id: int, error_code: int, **kwargs) -> QfRequest:
