...
class WikipediaTool(BaseTool):
    ...
    def __init__(self, max_results=3, wiki_max_length=1000, result_max_length=2000):
        """
    ...
    def run(self, parameters: Dict[str, str] = None) -> List[Dict[str, str]]:
        ...
        search_results = wikipedia.search(
            query, results=self.max_results
        )
        results = []
        ...
        for result in results:
            if total_length + len(result["content"]) > self.result_max_length:
                result["content"] = result["content"][:self.result_max_length - total_length]
                break
