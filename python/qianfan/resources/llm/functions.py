...
from typing import Any, Dict, AsyncIterator, Iterator, List, Optional, Union, Tuple
...
class Function(ChatCompletion):
    ...
    def do(
        ...
        **kwargs: Any,
        ...
        resp = super().do(**kwargs)
        return self._convert_function_call_response(resp)

    def _convert_function_call_response(self, resp):
        ...
        if action and action_input:
            resp.body["function_call"] = {"name":action.group(1), "arguments": action_input.group(1)}
            resp.body["result"] = ""
        return resp
    ...
    def _render_functions_prompt(self, functions: List[Dict]) -> str:
        functions_prompt = []
        for f in functions:
            functions_prompt.append(self._render_prompt(
    ...
    def _render_user_query_msg(self, msg: Dict, **kwargs) -> Dict:
        return {"role":"user", "content": self._render_prompt(_DEFAULT_FUNCTIONS_ENHANCE_PROMPT, _PROMPT_IDENTIFIER, 
